...
# aspect_dict = {'food':0, 'misc':1, 'location':2, 'ambience':3, 'drinks':4, 'service':5, 'price':6, 'restaurant':7}
...
def batch_process(batch):
	...
	#将数据类型转化成tensor
	aspect_list = torch.tensor(aspect_list_, dtype=torch.long)
	label_list = torch.tensor(label_list, dtype=torch.long)

	return text_list_.to(device), aspect_list.to(device), label_list.to(device)